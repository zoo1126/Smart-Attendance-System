...
findoutApply_sql='select * from message where m_type=2 and m_s_phone=%s'
findApplybyState_sql='select * from message where m_type=2 and m_state=%s and m_s_phone=%s'
findApplybyTime_sql='select * from message where m_type=2 and m_apply_datetime between %s and %s  and m_s_phone=%s'
...
def findAlloutApply(com_id):
    ...
    try:
        for i in stafflist:
            cursor.execute(findoutApply_sql, i[0])
            t= cursor.fetchall()
            list=list+t
        cursor.close()
        db.close()
        ...
        db.rollback()
        print("执行MySQL: %s 时出错：%s" % (findoutApply_sql, e))
        return None
...
def findApplybyPhone(phone,com_id):
    ...
    try:
        for i in stafflist:
            if phone==i[0]:
                cursor.execute(findoutApply_sql, phone)
                t = cursor.fetchall()
                list = list + t
        cursor.close()
        db.close()
